...
const Customizer = () => {
  ...
  const handleDecals = (type, result) => {
    ...
    if (!activeFilterTab[decaleType.FilterTabs]) {
      handleActiveFilterTab(decaleType.FilterTabs);
    }
  }
      ...
      <AnimatePresence>
        {!snap.intro && (
          <>
            ...
            >
              {FilterTabs.map((tab) => (
                <Tab key={tab.name} isFilterTab isActiveTab={activeFilterTab[tab.name]} tab={tab} handleClick={() => handleActiveFilterTab(tab.name)} />
              ))}
            </motion.div>
          </>
        )}
